...
const OTPInputBox = ({ index, verifyOTP, state }: OTPInputBoxProps) => {
    ...
    const onInput = (e: React.FormEvent<HTMLInputElement>) => {
        ...
        if (value.match(/^[0-9]$/)) {
            ...
            // target.value = value // React handles this
            if (index < 3) {
        ...
        // We need to wait for the state update or read values directly
    ...
    return (
        <motion.div
            className={`w-14 h-16 rounded-lg ring-2 ring-transparent focus-within:shadow-inner overflow-hidden transition-all duration-300 ${state === "error"
...
export function OTPVerification() {
    ...
    const errorAnimation = async () => {
        ...
        setTimeout(() => {
            // Only reset if it's still error and user hasn't typed fully yet?
